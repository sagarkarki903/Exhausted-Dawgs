  ...
  return (
    <>
          ...
          <div className="relative p-6 z-10">
            <motion.div className="flex gap-4 my-4 text-black px-4 py-2 w-56 items-center" whileHover={{ scale: 1.03 }}>
                <MoveLeft />
                <Link to="/" className='font-semibold text-xl hover:text-red-900'>Back to Home</Link>
            </motion.div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {dogs.map((dog, index) => (
                <motion.div key={index} className="rounded-lg border border-gray-300 shadow-md transition-shadow hover:shadow-2xl bg-background/80 backdrop-blur-sm" whileHover={{ scale: 1.03 }}>
                  <img
                  src={`/dog2.jpeg`}
                  alt={`Dog ${index}`}
                  width={50}
